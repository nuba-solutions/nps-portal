...
const DashboardStatsCard = ({title, subtitle, icon, content, color, currency}: TDashboardStatsCardProps) => {
    ...
    return (
        <div className={`flex items-center justify-between p-6 bg-white dark:bg-slate-800 rounded-lg md:rounded-xl shadow-xl shadow-slate-400/10 dark:shadow-slate-950/50`}>
            ...
            <div className='text-2xl font-semibold'>{currency ? formatAmountForDisplay(content as number / 100, "usd") : content}</div>
        </div>
    )
}
