...
const DashboardStatsCard = ({title, subtitle, icon, content, color, currency, isPending}: TDashboardStatsCardProps) => {
    ...
    return (
        <div className={`flex flex-col sm:flex-row sm:items-center sm:justify-between px-3 py-4 4xl:px-6 bg-white dark:bg-slate-800 rounded-lg md:rounded-xl shadow-xl shadow-slate-400/10 dark:shadow-slate-950/50`}>
            ...
            {
                !isPending ? (
                    <div className='text-base sm:text-lg xl:text-lg 4xl:text-2xl font-semibold self-end sm:self-center'>{currency ? formatAmountForDisplay(content as number / 100, "usd") : content}</div>
                ) : (
                    <p>NA</p>
                )
            }
